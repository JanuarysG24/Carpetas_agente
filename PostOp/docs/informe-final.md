...
## 9 · Pregunta 1 del video — cómo se lo vendería a un cliente

*Guion sugerido — a grabar frente a cámara, no leer textual:*
...
>
> Lo que construí llama al paciente, conversa en su idioma y su registro —no un formulario leído en voz alta—, y decide con dos mecanismos independientes si el caso necesita a una persona: uno es el juicio del modelo escuchando la conversación completa; el otro son reglas clínicas declaradas y auditables, que no dependen de que el modelo "se acuerde" de aplicarlas bien. Basta que uno de los dos diga que hay que escalar para que se escale — así el sistema nunca depende de que un solo mecanismo acierte siempre.
>
> El valor diferencial frente a un chatbot de síntomas genérico: cada respuesta clínica que da el agente está anclada a un documento real de la base de conocimiento, visible y auditable, y esa base se actualiza sin tocar código — un supervisor sube un protocolo nuevo y el agente lo usa en la siguiente llamada, lo retira y lo olvida. No es una demo que "suena" médica; es un sistema donde se puede rastrear por qué dijo lo que dijo.

---

## 10 · Pregunta 2 del video — la decisión técnica más relevante

*Guion sugerido:*

> La decisión más relevante fue **abandonar la arquitectura de capas separadas a mitad de la sesión y volver a un solo archivo sin servidor.**
>
> Alternativa evaluada: un monorepo con cuatro paquetes TypeScript (contratos, capa conversacional, capa determinista, capa de decisión), comunicados por contratos tipados y con 334 pruebas automatizadas ya en verde — arquitectónicamente más cercana a un sistema de producción real, con las cuatro capas que describe la letra del reto.
>
> Por qué la descarté para la entrega: el enlace entre esos paquetes depende de symlinks de `npm install`, y en el entorno de desarrollo sincronizado por OneDrive esos enlaces se corrompen al pasar de un sistema a otro — lo confirmé tratando de levantarlo y viendo el error `ERR_MODULE_NOT_FOUND` en vivo. La compuerta G2 exige un levantamiento de ≤15 minutos siguiendo solo el README, sin segunda oportunidad salvo por credenciales. Apostar la entrega completa a que ese enlace sobreviva en la máquina del jurado era un riesgo que no valía la pena correr contra un reloj que no perdona.
>
> Riesgo que sí acepté al simplificar: un solo archivo sin capas separadas es más difícil de extender en equipo y no tiene la garantía de tipos en tiempo de compilación que sí tiene la versión heredada.
>
> Con dos semanas más, no volvería a la arquitectura de paquetes separados por npm — el riesgo de sincronización es del entorno, no del diseño. Portaría el mismo diseño de contratos tipados a un único build (un bundle con Vite o esbuild, sin symlinks entre paquetes locales) para conservar la garantía de tipos sin el riesgo de instalación, y agregaría streaming de audio real en vez de push-to-talk por turnos.

---

## 11 · Qué falta / próximos pasos declarados

- Cifras reales de latencia/tokens/costo de una sesión de prueba completa (pendiente de ejecutar y pegar en el README, ver § Métricas).
- Capturas de pantalla del demo (marcadas como pendientes arriba).
- TTS con voces dedicadas en español (Piper/Kokoro) en vez de la voz del sistema operativo — mencionado como stack sugerido, no obligatorio; la voz del sistema es la opción que el propio material del reto valida como aceptable para una primera entrega.
- Streaming continuo de audio en vez de push-to-talk, si el formato de evaluación en vivo lo recompensa más que la fiabilidad del control manual.
- Correr las 8 familias restantes del banco adversarial (33 ataques) contra `plantilla-chat-voz.html` directamente, y corregir el defecto de trazabilidad encontrado en F9-02 (que la cita de un documento distinga "consultado" de "esta fuente sostiene esta afirmación").
